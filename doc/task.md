...
- [x] **チケットC**: request_id をログフィールドへ付与
  - 前提: チケットB 完了
  - Issue C を GitHub に作成し、TDD で実装
  - 実装対象: `middleware/error_handler.go`
  - 取得キー: `"request_id"`（string）— `RequestIDMiddleware` がセット
  - PR で Issue C を Close
  - 進捗メモ（2026-05-26）: request_id有無ケースのテスト追加完了。ログ出力でrequest_idとX-Request-IDヘッダの一致を確認。
...
6. 同一イベントの重複出力を避ける
7. DEBUGログは常設追加を前提にせず、INFOだけでは内部状態を追えない処理に必要時のみ追加する

### テスト項目
- 正常系フロー3つ（ログイン成功、商品一覧取得、注文作成）で必須フィールドが欠落なく出力されること
- request_id を起点に時系列追跡可能であること
- イベント重複が発生しないこと
- 現時点でDEBUGログを常設追加しない判断理由が記録されていること
```

- [ ] **チケットF**: 通常イベントログ（INFO/DEBUG）実装（P0）
  - 前提: チケット D 完了
  - [x] Issue F を GitHub に作成し、TDD で実装
  - 実装対象:
    - [x] `handler/user.go` — ログイン成功イベント出力
    - [x] `handler/product.go` — 商品一覧取得イベント出力
    - [x] `handler/order.go` — 注文作成イベント出力
  - 実装内容:
    - [x] slog で INFO レベルのイベントログを出力
    - [x] DEBUGログは全ハンドラ調査の結果、現時点では常設追加しない方針に整理
    - [x] 必須フィールド（request_id, event, method, route, status, duration_ms）を構造化フィールドとして付与
    - [x] 認証済みの場合は user_id も付与
    - [x] event 命名を snake_case で統一
    - [x] ハンドラ層では正常系イベント出力のみ、エラーは return で ErrorHandler に委譲
  - 受け入れ条件:
    - [x] 正常系フロー3つ（ログイン成功、商品一覧取得、注文作成）で必須フィールド（request_id, event, status, duration_ms）が欠落なく出力される
    - [x] ハンドラ層でのみイベント出力が実行される（ErrorHandler では出力されない）
    - [x] 同一イベントの重複出力がない
    - [x] ErrorHandler の異常系ログと重複していない（二重ログなし）
    - [x] DEBUGログは「INFOだけでは内部状態を追えない複数ステップ処理に必要時のみ追加」と判断し、現時点の常設追加は不要とする
  - クローズ条件（2026-06-02整理）:
    - [x] Issue F のPRで上記受け入れ条件を確認し、IssueをCloseする
    - [x] 必要に応じて、DEBUGログを常設追加しない判断をPR本文に明記する
...
### 受け入れ条件（#60 クローズ基準）
- [x] 正常系フロー（ログイン成功、商品一覧取得、注文作成）で request_id 起点に `request_id`・`event`・`method`・`route`・`status`・`duration_ms` が欠落なく出力されること
- [x] 異常系シナリオ（DB切断・バリデーションエラー）で JSON ログから 10 秒以内に原因特定できること
- [x] エラーが最上位（ErrorHandler）で重複なく構造化ログとして出力されていること
- [x] JWT署名・DBパスワード・PII（個人情報）がログに含まれていないこと
- [x] 各ログレベルの採用理由について自分の論理的根拠があること
...
### ADR 作成済み
設計書ベース・実装計画との整合性チェック完了。ADR を [doc/planning/adr-structured-logging-error-policy-2026-05-14.md](planning/adr-structured-logging-error-policy-2026-05-14.md) に作成。矛盾・不足 7 件を指摘、是正アクション 8 件を記載。
