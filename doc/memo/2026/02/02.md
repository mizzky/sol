...
- 依存性注入の導入：`LoginUserHandler` に `auth.TokenGenerator` を注入し、トークン生成エラーのテストを実現
...
### 優先度1：`RegisterUserHandler` のテストコード作成
- `RegisterUserHandler` のテストケースを作成
...
- **依存性注入**: トークン生成のテストを可能にするため、`LoginUserHandler` に `auth.TokenGenerator` を注入