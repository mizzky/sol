...
- 例:
  ...
  {
    "id": 1,
    登録済みのメールアドレスとパスワードで認証を行い、成功時にアクセストークンとリフレッシュトークンを HttpOnly Cookie で返します。
    "description": null
  }
  ```
...
### エラーレスポンスの例
    ...
    レスポンス JSON には認証トークンは含まれません（トークンは HttpOnly Cookie で返却されます）。
    ```json
    {
      "message": "ログイン成功！",
      "user": {
    ...
    注: アクセストークンは HttpOnly Cookie（`access_token`）で返され、リフレッシュトークンは HttpOnly Cookie（`refresh_token`）で返されます。クライアントは `credentials: 'include'` を指定してリクエストを行ってください。
