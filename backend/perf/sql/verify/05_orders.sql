...
DECLARE
    ...
    expected_user1_orders BIGINT;
    expected_pending_orders BIGINT;
    expected_cancelled_orders BIGINT;
...
BEGIN
    ...
    expected_user1_orders := expected_orders_count * 20 / 100;
    expected_pending_orders := expected_orders_count * 90 / 100;
    expected_cancelled_orders := expected_orders_count * 10 / 100;

    SELECT count(*), min(id), max(id)
    INTO actual_orders, min_order_id, max_order_id
    FROM public.orders;

    IF actual_orders <> expected_orders_count
        OR min_order_id IS DISTINCT FROM 1
        OR max_order_id IS DISTINCT FROM expected_orders_count THEN
            RAISE EXCEPTION
                'orders range mismatch: expected count/min/max %/1/%,  actual %/%/%',
    ...
    IF user1_orders <> expected_user1_orders THEN
        RAISE EXCEPTION
            'user_id=1 orders mismatch: expected %, actual %',
            expected_user1_orders,
            user1_orders;
    END IF;
    ...
    IF pending_orders <> expected_pending_orders OR cancelled_orders <> expected_cancelled_orders THEN
        RAISE EXCEPTION
            'status distribution mismatch: expected pending/cancelled %/%, actual %/%',
            expected_pending_orders,
    ...
    IF orders_sequence_value <> expected_orders_count OR NOT orders_sequence_called THEN
        RAISE EXCEPTION
